...
func _process(delta: float) -> void:
	# Approach player
	position.y += Global.vertical_speed * delta
	scale += Vector2(Global.scale_speed * delta * 0.01, Global.scale_speed * delta * 0.01)
	...
	if position.y > get_viewport_rect().size.y * 0.55: # Adjust value based on where your player is
		queue_free()
